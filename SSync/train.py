...
from SSync import configuration, data, metrics, models, utils
...
def _setup_callbacks(args, config, log_path: pathlib.Path, dataset=None) -> Dict[str, pl.Callback]:
    ...
    if not args.dry:
        ...
        checkpointer = pl.callbacks.ModelCheckpoint(
            ...
            filename=f"{config.experiment_group}_step={{step}}", # filename="{step}",
...
def _setup_loggers(args, config, log_path: pathlib.Path) -> Dict[str, pl.loggers.logger.Logger]:
    ...
    if not args.no_tensorboard:
        ...
        loggers["tensorboard"] = pl.loggers.TensorBoardLogger(
            save_dir=log_path, name=TENSORBOARD_SUBDIR, version=config.experiment_group
        )
    # config.experiment_name,

    # CSV logs go to <log_dir>/<metrics_subdir>/version_N/metrics.csv, where N is the number of
    # restarts of the job
    loggers["csv"] = pl.loggers.CSVLogger(save_dir=log_path, name=METRICS_SUBDIR, version=config.experiment_group)
...
def main(args, config_overrides=None):
    ...
    if not args.dry:
        ...
        print(f"Using {log_path} as output directory")
    ...
    if args.continue_from:
        if args.continue_from.is_file() and args.continue_from.suffix == ".ckpt":
            ...
            ckpt_path = utils.find_last_checkpoint(args.continue_from, config.experiment_group)
        else:
            raise ValueError("Unclear --continue argument: should be .ckpt file or directory")
    elif not args.dry:
        ckpt_path = utils.find_last_checkpoint(log_path, config.experiment_group)
    ...
    if rank_zero and log_path and not (log_path / "settings" / config.experiment_group / "settings.yaml").exists():
        os.makedirs(log_path / "settings" / config.experiment_group, exist_ok=True)
        configuration.save_config(log_path / "settings" / config.experiment_group / "settings.yaml", config)
