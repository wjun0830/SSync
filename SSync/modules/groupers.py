...
from SSync.modules import networks
...
class SlotAttention(nn.Module):
    ...
    def forward(self, slots: torch.Tensor, features: torch.Tensor, n_iters: Optional[int] = None):
        features = self.norm_features(features) # 1 7 64
        keys = self.to_k(features) # (1 30 1369 64)
        values = self.to_v(features) # (1 30 1369 64)
        ...
        return {"slots": slots, "masks": pre_norm_attn, "dots": dots}
